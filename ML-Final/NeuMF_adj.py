...
class MLP(nn.Module):
  def __init__(self, num_users, num_items, layers, reg_layers, dropout=0.3):
    ...
    for i in range(1, self.num_layers):
      ...
      mlp_layers.append(nn.LayerNorm(layers[i]))
...
class NeuMF(nn.Module):
  def __init__(self, num_users, num_items, mf_dim, layers, reg_layers, reg_mf, dropout=0.3):
  ...
  def _init_weights(self):
    nn.init.normal_(self.mf_user_embeddings.weight, std=0.01)
    nn.init.normal_(self.mf_item_embeddings.weight, std=0.01)
    nn.init.xavier_uniform_(self.mlp_user_embeddings.weight)
    nn.init.xavier_uniform_(self.mlp_item_embeddings.weight)
    for layer in self.mlp_layers:
      if isinstance(layer, nn.Linear):
        nn.init.xavier_uniform_(layer.weight)
        nn.init.zeros_(layer.bias)
...
if __name__ == "__main__":
  ...
  model = NeuMF(num_users, num_items, args.num_factors, layers, reg_layers, args.reg_mf, args.dropout).to(device)
  ...
  if args.out > 0:
    best_model = NeuMF(num_users, num_items, args.num_factors, layers, reg_layers, args.reg_mf, args.dropout).to(device)
