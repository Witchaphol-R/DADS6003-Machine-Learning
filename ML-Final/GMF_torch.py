...
class GMFDataset(Dataset):
  ...
  def __getitem__(self, index):
    ...
    if label == 0:
      ...
      # Get positive item from the sparse matrix
...
def evaluate(model, test_data, top_k):
  ...
  Args:
    model: The trained GMF model.
    test_data: A list of tuples (user, item) representing test interactions.
...
if __name__ == "__main__":
  ...
  parser.add_argument('--verbose', type=int, default=0, help='Show performance per X iterations')
  parser.add_argument('--top_k', type=int, default=10, help='Number of top recommendations to consider.')
  parser.add_argument('--out', type=int, default=0, help='Whether to save the trained model.')
  ...
  train_dataset = GMFDataset(dataset.trainMatrix, args.num_neg)
  num_users = train_dataset.num_users
  num_items = train_dataset.num_items
  regs = eval(args.regs) 
  model = GMF(num_users, num_items, args.num_factors, regs).to(device)
  ...
  for epoch in range(args.epochs):
    ...
    for user_ids, item_ids, labels in train_dataloader:
      ...
      loss += regs[0] * torch.norm(model.user_embeddings.weight) + regs[1] * torch.norm(model.item_embeddings.weight) # Add reg loss
      loss.backward()
      optimizer.step()
    ...
    if hr > best_hr:
      best_hr = hr
      if args.out > 0: 
        torch.save(model.state_dict(), 'BestModel/best_gmf_model.pth')

  if args.out > 0:
    best_model = GMF(num_users, num_items, args.num_factors, regs).to(device)
    best_model.load_state_dict(torch.load('BestModel/best_gmf_model.pth'))
